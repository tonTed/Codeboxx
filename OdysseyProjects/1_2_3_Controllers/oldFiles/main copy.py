...
class Column:
	...
	def _createCallButtonsInFloors(self):					#[X]
		"""
		...
		for floor in range(1, self.floors + 1):
			if floor == 1 :
				self.callButtonsInFloors.append({"direction": "up", "floor": floor, "pressed": False})
			elif floor == self.floors + 1:
				self.callButtonsInFloors.append({"direction": "down", "floor": floor, "pressed": False})
			else:
				self.callButtonsInFloors.append({"direction": "up", "floor": floor, "pressed": False})
				self.callButtonsInFloors.append({"direction": "down", "floor": floor, "pressed": False})
	...
	def run(self):
		for button in self.callButtonsInFloors:
